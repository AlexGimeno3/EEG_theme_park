...
class ViewSpectrogram(EEGFunction):
    name = "View spectrogram"
    params_units_dict = {"save_path": "path or None", "ext": "", "dpi": "dots per inch", "figsize": "inches (w, h)"}

    def __init__(self, save_path=None, ext=".png", dpi=300, figsize=(12, 4), **kwargs):
        """
        Displays a spectrogram of the signal in the selected time range.

        Inputs:
        - save_path (str or None): if not None, saves the figure to this directory
        ...
        # Parse figsize if passed as a string from the GUI
        if isinstance(self.figsize, str):
    ...
    def _apply_function(self, original_signal, eeg_object, **kwargs):
        """
        Does not modify the signal. On the first call per apply() invocation,
        generates and displays a spectrogram for the current channel over the
        full analysis time range (pulling data from eeg_object directly, rather
        than using original_signal, which may be a single clean segment).
        """
        if self._has_shown:
        ...
        # Determine the full analysis time range
        if len(eeg_object.analyze_time_lims) > 0:
            time_range = eeg_object.analyze_time_lims
        else:
            time_range = [eeg_object.times[0], eeg_object.times[-1]]
        ...
        # Pull full-range data for the current channel directly
        ch_name = eeg_object.current_channel
        data = eeg_object.all_channel_data[ch_name][start_idx:end_idx + 1]
        srate = eeg_object.srate

        # Replace NaNs with zeros for FFT
        data_clean = np.where(np.isnan(data), 0, data)

        # Compute spectrogram
        nperseg = min(int(srate * 2), len(data_clean))  # 2-second windows, capped by data length
        if nperseg < 4:
            simple_dialogue("Not enough data in the selected range to compute a spectrogram.")
            return original_signal

        noverlap = nperseg // 2
        freqs, times_spec, Sxx = scipy_spectrogram(
            data_clean, fs=srate, nperseg=nperseg, noverlap=noverlap
        )

        # Offset spectrogram times to align with signal time axis
        times_spec = times_spec + time_range[0]

        # Plot
        fig, ax = plt.subplots(figsize=self.figsize)
        Sxx_log = 10 * np.log10(Sxx + 1e-12)
        pcm = ax.pcolormesh(times_spec, freqs, Sxx_log, shading='gouraud', cmap='viridis')
        ax.set_ylabel('Frequency (Hz)')
        ax.set_xlabel('Time (s)')
        ax.set_title(f'Spectrogram: {eeg_object.name} — Channel: {ch_name}')
        fig.colorbar(pcm, ax=ax, label='Power (dB)')

        # Cap frequency axis at a reasonable EEG range
        max_freq = min(srate / 2, 80)
        ax.set_ylim(0, max_freq)

        if self.save_path is not None:
            save_full = Path(self.save_path) / f"{eeg_object.name}_spectrogram{self.ext}"
            fig.savefig(save_full, dpi=self.dpi, bbox_inches='tight')
            print(f"Spectrogram saved to {save_full}")
