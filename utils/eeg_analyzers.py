...
class RelativeDeltaPower(MultiChannelAnalyzer):
    """
    Computes relative delta power (1–4 Hz) as a fraction of total power (1–40 Hz),
    averaged across all channels, using multitaper spectral estimation (DPSS tapers).
    ...
    def _apply_function_multi(self, window_signals, eeg_object, **kwargs):
        ...
        delta_in_fpass = (freqs_in_fpass > 1) & (freqs_in_fpass < 4)

        rdp = np.mean(all_channels_S[delta_in_fpass]) / np.mean(all_channels_S)
        return float(rdp)
    
class RelativeThetaPower(MultiChannelAnalyzer):
    """
    This mirrors the 'rdp' feature from wrt_calcFeatures.m:
        allchsavgS = mean([S_ch1, S_ch2, ...], 2)
        rdp = mean(allchsavgS(f>1 & f<4)) / mean(allchsavgS)
    ...
    def _apply_function_multi(self, window_signals, eeg_object, **kwargs):
        ...
        # Relative delta power: mean(S in delta) / mean(S in fpass)
        freqs_in_fpass = freqs[fpass_mask]
        delta_in_fpass = (freqs_in_fpass > 4) & (freqs_in_fpass < 8)

        rdp = np.mean(all_channels_S[delta_in_fpass]) / np.mean(all_channels_S)
        return float(rdp)

class RelativeAlphaPower(MultiChannelAnalyzer):
    """
    This mirrors the 'rdp' feature from wrt_calcFeatures.m:
        allchsavgS = mean([S_ch1, S_ch2, ...], 2)
        rdp = mean(allchsavgS(f>1 & f<4)) / mean(allchsavgS)
    ...
    def _apply_function_multi(self, window_signals, eeg_object, **kwargs):
        ...
        # Relative delta power: mean(S in delta) / mean(S in fpass)
        freqs_in_fpass = freqs[fpass_mask]
        delta_in_fpass = (freqs_in_fpass > 8) & (freqs_in_fpass < 13)

        rdp = np.mean(all_channels_S[delta_in_fpass]) / np.mean(all_channels_S)
        return float(rdp)

class RelativeBeta1Power(MultiChannelAnalyzer):
    """
    This mirrors the 'rdp' feature from wrt_calcFeatures.m:
        allchsavgS = mean([S_ch1, S_ch2, ...], 2)
        rdp = mean(allchsavgS(f>1 & f<4)) / mean(allchsavgS)
    ...
    def _apply_function_multi(self, window_signals, eeg_object, **kwargs):
        ...
        # Relative delta power: mean(S in delta) / mean(S in fpass)
        freqs_in_fpass = freqs[fpass_mask]
        delta_in_fpass = (freqs_in_fpass > 13) & (freqs_in_fpass < 25)

        rdp = np.mean(all_channels_S[delta_in_fpass]) / np.mean(all_channels_S)
        return float(rdp)

class RelativeBeta2Power(MultiChannelAnalyzer):
    """
    This mirrors the 'rdp' feature from wrt_calcFeatures.m:
        allchsavgS = mean([S_ch1, S_ch2, ...], 2)
        rdp = mean(allchsavgS(f>1 & f<4)) / mean(allchsavgS)
    ...
    def _apply_function_multi(self, window_signals, eeg_object, **kwargs):
        ...
        # Relative delta power: mean(S in delta) / mean(S in fpass)
        freqs_in_fpass = freqs[fpass_mask]
        delta_in_fpass = (freqs_in_fpass > 25) & (freqs_in_fpass < 40)
